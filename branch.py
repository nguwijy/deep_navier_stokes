...
if __name__ == "__main__":
    ...
    kappa_map = np.array([-1, -1, -1, 0, 1, 2, 0, 0, 0, 1, 1, 1, 2, 2, 2])
    ...
    deriv_condition_kappa_map = np.array([0, 1, 2])
    ...
    def g_fun(x, i):
        if i == 0:
            return A * sin(x[2]) + C * cos(x[1])
        elif i == 1:
            return B * sin(x[0]) + A * cos(x[2])
        else:
            return C * sin(x[1]) + B * cos(x[0])
    ...
    model = Net(
        ...
        deriv_condition_kappa_map=deriv_condition_kappa_map,
        f_fun=f_fun,
        phi_fun=g_fun,
        T=T,
        x_lo=x_lo,
    ...
    # plt.plot(grid, model(nn_input.to(device)).detach().cpu(), label="Deep branching")
